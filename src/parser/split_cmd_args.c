...
/*   Updated: 2022/03/12 11:24:03 by ahel-mou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	cpystr_w_char(char *cmd, char *arr, int i)
{
	int	j;
...
static char	**fill_arr(int numof_words, char *cmd, char **arr)
{
	...
	while (k < numof_words)
	{
		...
		while (cmd[i] && white_spaces(cmd[i])) // here we skip the spaces between words
			i++;
		if (cmd[i] == '\'')
...
static int	word_count(t_shell *sh, char *cmd)
{
	...
	while (cmd[i])
	{
		if (cmd[i] == '\'')
		{
			i = go_next_quote(sh, cmd, i, '\''); // we skip anything between quotes, return an index of the closing quote
			count++;
		}
		else if (cmd[i] == '"')
		{

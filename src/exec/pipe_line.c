...
/*   Updated: 2022/03/13 13:21:27 by ahel-mou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	parent_process(t_shell *shell, int i)
{
	...
	if (i == shell->num_of_cmds - 1)
	{
		...
		switch_pipe(shell, shell->pipe_fd);
	close_fds(shell);
	free_redir(shell);
}
...
static void	cmd_execution(t_shell *shell, int i)
{
	int		j;
	char	*cmd;
	...
	if (shell->cmd_with_args[0] && !shell->error_skip) 
	...
	while (!shell->error_skip && shell->cmd_with_args[0] && shell->path[j])
	{
...
void	pipe_line(t_shell *shell)
{
	...
	while (shell->cmd[++i])
	{
		...
		if (g_proc == 0) // start child process
			cmd_execution(shell, i);
		else if (g_proc > 0)
			parent_process(shell, i);

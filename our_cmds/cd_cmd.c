...
/*   Updated: 2022/03/21 13:47:57 by ahel-mou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	back_to_home(t_shell *shell, char *home_dir)
{
	...
	{
		ft_putstr_fd("cd command failed", 2);
		shell->cmdretval = errno;
	}
	free(pwd);
}
...
static void	cd_redirection(t_shell *shell, char *cmd, char *option, char *home_dir)
{
	...
	{
		if (!option) // here we check if the cd option is NULL if yes we cd to HOME "cd"
...
void	cd_cmd(t_shell *shell)
{
	...
	if (shell->num_of_cmds > 1)  // we cant cd when there is a pipe so we skip cd
		return ;
	home_dir = get_envar_value(shell, "HOME"); // we store the home path | we get it from the env_vars
	if (!home_dir)
	{

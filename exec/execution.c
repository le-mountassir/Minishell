...
/*   Updated: 2022/03/21 17:41:28 by ahel-mou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	export_env(t_shell *shell, int cmd_id)
{
	int i = 0;

	if (starts_with("env", shell->cmd[cmd_id])) // checks if the command is env if yes it prints our_envirement
		while (shell->our_env[i])
			printf("%s\n", shell->our_env[i++]);
	else if (starts_with("export", shell->cmd[cmd_id])) // if the args are the same it displays the export 
		display_export(shell);
	else if (starts_with("echo", shell->cmd[cmd_id]))
		echo_cmd(shell->cmd_with_args);
...
static void	cmd_execution(t_shell *shell, int i, int *pipe_fd)
{
	...
	while (!shell->input_error && shell->cmd_with_args[0] && shell->path[j] && check)
	{
...
void	execution(t_shell *shell)
{
	...
	while (shell->cmd[++i])
	{
		...
		// our homemade_cmd ar not pipeable execpt echo and pwd means we just skip them
		// heredoc with no command after it is not pipeable
		if (only_hrdoc_with_nocmd(shell) || not_executable_cmd(shell, shell->cmd_with_args[0]))
		{
		...
		if (cld_proc < 0)
			fork_failed(shell);
		if (cld_proc == 0) // start child process
